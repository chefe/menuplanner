            ...
            <div class="bg-white p-3 rounded-b">
                <form class="form-horizontal" method="POST" action="{{ route('register') }}">
                    ...
                    <div class="flex items-stretch mb-3">
                        <label for="name" class="text-right font-semibold text-gray-600 text-sm pt-2 pr-3 align-middle w-1/4">@lang('app.name')</label>
                        <div class="flex flex-col w-3/4">
                            <input id="name" type="text" class="flex-grow h-8 px-2 border rounded {{ $errors->has('name') ? 'border-red-600' : 'border-gray-400' }}" name="name" value="{{ old('name') }}" autofocus>
                            {!! $errors->first('name', '<span class="text-red-600 text-sm mt-2">:message</span>') !!}
                        </div>
                    </div>
                    ...
                    <div class="flex items-stretch mb-3">
                        ...
                        <div class="flex flex-col w-3/4">
                            <input id="email" type="email" class="flex-grow h-8 px-2 border rounded {{ $errors->has('email') ? 'border-red-600' : 'border-gray-400' }}" name="email" value="{{ old('email') }}" required>
